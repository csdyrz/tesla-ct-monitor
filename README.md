...
- 🚫 **查询必须带"可交付过滤"参数**(`region` + `isFalconDeliverySelectionEnabled` + `version:v2`,官网同款):特斯拉库存车有**销售区域锁定**,裸查询会混入大量"列表可见、但下单页提示 *not available for your registration ZIP Code*"的车(实测 2026-07-30:裸查询 13 辆,过滤后真正可下单的只有 1 辆)。本监控只推送**你的邮编真正能下单**的车。
...
| `ZIP_REGION` | `CA` | 邮编所在州代码,官网"可交付/可注册"过滤必需;**换邮编时必须同步改** |
...
- 😶 **一直没消息** → 大概率是真没货:可下单库存被区域锁定切割后非常少(写此文时 92614 可下单的真·全新 AWD 为 **0 辆**);跑 `--dry-run` 看当前匹配结果;确认 Server酱当日 5 条额度没用完。
